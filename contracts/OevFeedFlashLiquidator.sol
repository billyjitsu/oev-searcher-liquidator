...
contract OevFlashLiquidator is Ownable, IApi3ServerV1OevExtensionOevBidPayer, IFlashLoanReceiver {
    ...
    function onOevBidPayment(
        ...
        // Execute flash loan with liquidation parameters
    ...
    function executeOperation(
        ...
        // Approve lending pool to use flash loaned amount
        IERC20(assets[0]).approve(address(lendingPool), amounts[0]);
        ...
        // Swap the received collateral back to debt asset using the DEX
        if (collateralReceived > 0) {
            IERC20(liquidationParams.collateralAsset).approve(address(dex), collateralReceived);
            dex.swap(liquidationParams.collateralAsset, liquidationParams.debtAsset, collateralReceived);
        }
